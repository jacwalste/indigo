...
class WillowsScript(Script):
    ...
    def __init__(self, ctx: ScriptContext, max_hours: float = 6.0,
        ...
        super().__init__(
            config=ScriptConfig(
                name="Willows", max_runtime_hours=max_hours,
    ...
    def on_start(self) -> None:
        ...
        self._log(f"Chopping willows ({mode}) - ensure NPC Indicators (cyan) + deposit box (red)")
    ...
    def _do_click_tree(self) -> None:
        ...
        if not self.is_target_near(TREE_COLOR, (x, y)):
            self._log("Willow gone before click (stale target), re-finding")
            self._state = State.FIND_TREE
            return
        ...
        self._log(f"Clicked willow (inv={inv_count}), verifying chop (timeout={timeout:.0f}s)...")
