...
from indigo.core.timing import NORMAL_ACTION, CAREFUL_ACTION
...
class OaksScript(Script):
    ...
    IDLE_TIMEOUT_MIN = 15.0
    IDLE_TIMEOUT_MAX = 25.0

    def __init__(self, ctx: ScriptContext, max_hours: float = 6.0,
                 on_log: Optional[Callable[[str], None]] = None):
        super().__init__(
            config=ScriptConfig(name="Oaks", max_runtime_hours=max_hours),
            ctx=ctx,
            on_log=on_log,
        )
        ...
        self._idle_timeout = 16.0
        self._find_failures = 0
        self._logs_banked = 0
        self._bank_trips = 0

    def on_start(self) -> None:
        self._log("Chopping oaks (bank mode) - ensure NPC Indicators (cyan) + deposit box (red)")
    ...
    def _do_click_tree(self) -> None:
        if self._last_target:
            ...
            self._idle_timeout = self.ctx.rng.truncated_gauss(
                mean=20.0, stddev=3.0,
                min_val=self.IDLE_TIMEOUT_MIN, max_val=self.IDLE_TIMEOUT_MAX,
            )
            self._wait_checks = 0
            self._state = State.WAITING
            self._log(f"Clicked oak (inv={self._last_inv_count})")
