"""
Oak Trees Script (with Deposit Box Banking)

Chops oak trees using NPC Indicators (cyan highlight).
...
Requires:
- NPC Indicators: cyan (#00FFFF) on oak trees
...
# XP drop verification timeout — must account for walk time to tree
CHOP_XP_TIMEOUT = 14.0

# Max seconds between XP drops before assuming tree fell
DROP_GAP_TIMEOUT = 14.0


class OaksScript(Script):
    """Chop oak trees and bank via deposit box."""
    ...
    def __init__(self, ctx: ScriptContext, max_hours: float = 6.0,
        ...
        super().__init__(
            config=ScriptConfig(
                name="Oaks", max_runtime_hours=max_hours,
    ...
    def on_start(self) -> None:
        ...
        self._log(f"Chopping oaks ({mode}) - ensure NPC Indicators (cyan) + deposit box (red)")
    ...
    def _do_find_tree(self) -> None:
        ...
        if tree:
            ...
            self._log(f"Found oak at {tree.click_point} (area={tree.area})")
        else:
            self._find_failures += 1
            if self._find_failures >= 3:
                self._log("No oak after 3 tries, searching...")
                ...
                if tree:
                    ...
                    return
                self._find_failures = 0
            self._log("No oak tree found, waiting...")
    ...
    def _do_click_tree(self) -> None:
        ...
        if not self.is_target_near(TREE_COLOR, (x, y)):
            self._log("Oak gone before click (stale target), re-finding")
            self._state = State.FIND_TREE
            return
        ...
        self._log(f"Clicked oak (inv={inv_count}), verifying chop (timeout={timeout:.0f}s)...")
        ...
        # Activity burst after clicking — player naturally fidgets after initiating action
    ...
    def _do_waiting(self) -> None:
        ...
        # Quick XP drop check first — fast and doesn't delay the gap timer
        ...
        # Periodic inventory check for full inventory (don't need it every tick)
        self._wait_checks += 1
    ...
    def _do_find_bank(self) -> None:
        ...
        if bank:
            ...
            self.ctx.delay.sleep_range(1.5, 3.0)
            # No idle during banking — zoom/scroll can interfere with deposit interface
    ...
    def _do_click_bank(self) -> None:
        ...
        # Poll for deposit box to open instead of blind sleep
        # No idle during banking — zoom/scroll can scroll inside the deposit interface
    ...
    def _do_depositing(self) -> None:
        # Verify deposit box interface is actually open (if template available)
        ...
        # Verify inventory actually emptied (accounting for reserved slots)
